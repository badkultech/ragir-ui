// components/library/AddNewItemModal/StepSelector.tsx
import { HelpCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { DialogClose, DialogFooter } from "@/components/ui/dialog";

export function StepSelector({ selected, setSelected, onNext, categories }: any) {
  return (
    <>
      <div className="grid grid-cols-2 gap-4 mt-4">
        {categories.slice(0, 6).map(({ label, icon: Icon, step }: any) => (
          <button
            key={label}
