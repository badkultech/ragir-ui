...
import { organizerState, setChooseModalOpen, setCityInput, setCityTags, setFormData, setLeaderModalOpen, setLeaders, setSelectedGroupLeaderId, setSelectedTags } from '@/app/organizer/-organizer-slice';
...
export function CreateTrip({ tripId }: Props) {
    ...
    const { leaderModalOpen, selectedTags, chooseModalOpen, selectedGroupLeaderId, leaders, cityInput, formData ,cityTags} = state;
    ...
    const [triggerGetTrip, { data: tripData, isFetching }] = useLazyGetTripByIdQuery();


    const handleFetchTrip = async (tripId: string) => {
        try {
            if (!organizationId || !tripId) {
                console.error("Missing organizationId or tripId");
                return;
            }

            const response = await triggerGetTrip({ organizationId, tripId }).unwrap();
    ...
    const handleSaveTrip = async () => {
        try {
            ...
            if (tripId) {
                ...
                localStorage.setItem("createdTripId", tripId);

                await handleFetchTrip(tripId);

                // 🔸 Option 2: Pass via query param
                router.push(
                    `/organizer/create-trip/${tripId}/Itinerary?tripId=${tripId}&startDate=${encodeURIComponent(
                        formData.startDate
    ...
    const toggleTag = (tag: string) => {
        dispatch(setSelectedTags(selectedTags.includes(tag) ? selectedTags.filter((t) => t !== tag) : [...selectedTags, tag]));
    };
    ...
    return (
        <div className='flex min-h-screen bg-gray-50'>
            <div className='flex-1'>
                            ...
                            <div className='space-y-3'>
                                <div className='flex gap-2'>
                                    <Input
                                        ...
                                        value={cityInput}
                                        onChange={(e) => { dispatch(setCityInput(e.target.value)) }}
                                ...
                                {cityTags.length > 0 && (
                                    <div className='flex flex-wrap gap-2'>
                                        {cityTags.map((tag) => (
                                            <span
                                                key={tag}
                                                onClick={() => { dispatch(setCityInput(tag)) }} // ✅ click to move tag text back to input
                    ...
                    <div className='max-w-auto mx-auto bg-white shadow rounded-2xl p-8 mt-4 '>
                        <div className='flex items-center justify-between mb-1'>
                            ...
                            <div className='flex gap-3'>
                                ...
                                <Button
                                    onClick={() => { dispatch(setChooseModalOpen(true)) }}
                ...
                {/* ✅ Add Leader Modal */}
                <Dialog open={leaderModalOpen} onOpenChange={(data) => {
                ...
                <LibrarySelectModal
                    ...
                    onClose={() => { dispatch(setChooseModalOpen(false)) }}
